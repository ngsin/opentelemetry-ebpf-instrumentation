...
// When accumulated_buf is non-NULL it points to a kernel-side buffer of size
// FULL_BUF_SIZE that already contains the HTTP request data (from SSL_read
// accumulation).  The function uses it directly instead of bpf_probe_read
// from user-space u_buf.  bytes_len should be set to the amount of valid
// data in the accumulated_buf (typically FULL_BUF_SIZE).
...
http_get_or_create_trace_info(http_connection_metadata_t *meta,
                              ...
                              const unsigned char *accumulated_buf) {
    //TODO use make_key
    egress_key_t e_key = {
        .d_port = conn->d_port,
        .s_port = conn->s_port,
    ...
    if (g_bpf_traceparent_enabled && !skip_tp_parsing) {
        ...
        if (!capture_header_buffer) {
            // Note: without capture_header_buffer, we cannot read the
            // traceparent from SSL-decoded HTTP data, so we cannot
            // determine the curl hook's span_id.  The span_id remains
            // as generated by urand_bytes().
            if (meta) {
        ...
        if (buf) {
            // Guard for BPF verifier: ensure bytes_len is non-negative
            // before the bitmask, so R2 of bpf_probe_read is proven >= 0.
            if (bytes_len < 0) {
                return;
            }
            ...
            if (accumulated_buf) {
                // Data is in a kernel-side accumulated buffer (from SSL_read
                // byte-at-a-time accumulation).  Copy directly instead of
                // reading from user-space.  The accumulated buffer is
                // FULL_BUF_SIZE (256) bytes; only the first bytes_len bytes
                // are valid data, the rest are zero-initialized.
                // Traceparent scanning over this small buffer will typically
                // not find the header; trace context propagation relies on
                // find_trace_for_server_request() or subsequent reads.
                __builtin_memcpy(buf, accumulated_buf, FULL_BUF_SIZE);
            } else {
            ...
            if (res) {
                ...
                } else if (meta && meta->type == EVENT_HTTP_CLIENT && ssl) {
                    // For SSL client requests: the C++ agent curl hook injected
                    // a traceparent header with a specific span_id.  Use that
                    // span_id as this client span's span_id so it matches the
                    // parent-id seen by the downstream server.
                    // Reading directly from the HTTP header is authoritative and
                    // immune to traces_ctx_v1 being overwritten by intervening
                    // TCP-level spans (TLS handshake).
                    bpf_dbg_printk("SSL client: using span_id from traceparent header");
...
static __always_inline void finish_http(http_info_t *info, pid_connection_info_t *pid_conn) {
    if (http_info_complete(info) && !info->submitted) {
        ...
        if (trace) {
            ...
            // Late-binding trace lookup for SSL server spans.
            // The C++ agent writes parsed traceparent from HTTPS headers into
            // incoming_trace_map after complete header reassembly (~200 SSL_reads).
            // At request detection time (SSL_read accumulation), the map was empty.
            // By response time (SSL_write), the agent has written the entry.
            if (trace->ssl && trace->type == EVENT_HTTP_REQUEST) {
                connection_info_t sorted_conn;
                __builtin_memcpy(&sorted_conn, &trace->conn_info, sizeof(connection_info_t));
                sort_connection_info(&sorted_conn);
                tp_info_pid_t *late_tp = bpf_map_lookup_elem(&incoming_trace_map, &sorted_conn);
                if (late_tp) {
                    bpf_dbg_printk("Late-binding: found agent traceparent for SSL server span");
                    __builtin_memcpy(trace->tp.trace_id, late_tp->tp.trace_id, sizeof(trace->tp.trace_id));
                    __builtin_memcpy(trace->tp.parent_id, late_tp->tp.span_id, sizeof(trace->tp.parent_id));
                    ...
                    if (srv_tp) {
                        ...
                        __builtin_memcpy(srv_tp->tp.span_id, trace->tp.span_id, sizeof(srv_tp->tp.span_id));
                        bpf_dbg_printk("Late-binding: updated server_traces for child span propagation");
                    }

                    // Also update traces_ctx_v1 so the C++ agent curl hook
                    // reads the correct traceID for HTTPS client requests.
                    const u64 lb_id = bpf_get_current_pid_tgid();
                    obi_ctx__set(lb_id, &trace->tp);
                }
            }
            ...
            // could read the correct traceID during this request's processing.
...
static __always_inline int __obi_continue2_protocol_http(struct pt_regs *ctx,
    ...
    if (meta) {
        ...
        if (tp_p) {
            info->tp = tp_p->tp;
            if (g_bpf_debug) {
                unsigned char dbg_tp_buf[TP_MAX_VAL_LENGTH];
                make_tp_string(dbg_tp_buf, &info->tp);
                bpf_dbg_printk("continue2 info->tp: %s", dbg_tp_buf);
            }
    ...
    if (args->accumulated) {
        // Data came from SSL_read accumulation — small_buf has the full
        // FULL_BUF_SIZE bytes of accumulated data, but u_buf points to a
        // tiny user-space buffer that cannot be safely read for the full
        // request.  Copy the accumulated data into info->buf and skip the
...
__obi_continue_protocol_http(struct pt_regs *ctx,
    ...
    http_get_or_create_trace_info(meta,
                                  ...
                                  &args->pid_conn.conn,
                                  (void *)args->u_buf,
                                  args->accumulated ? FULL_BUF_SIZE : args->bytes_len,
                                  args->ssl,
                                  args->orig_dport,
                                  tp_loop_fn,
                                  args->accumulated ? args->small_buf : NULL);
...
__obi_protocol_http(struct pt_regs *ctx, unsigned char *(*tp_loop_fn)(unsigned char *, const u16)) {
    ...
    if (args->packet_type == PACKET_TYPE_REQUEST && (info->status == 0) &&
        ...
        // Append incoming byte(s) to info->buf so the full URL is captured.
        // This is critical for byte-at-a-time SSL_read (e.g. cpp-httplib):
        // the initial accumulation dispatch only fills the first MIN_HTTP_SIZE
        // bytes; subsequent reads must continue building the URL here.
        //
        // Stop appending once buf is full.  The bitmask (& (FULL_BUF_SIZE-1))
        // wraps around at 256, which would overwrite the request line at
        // buf[0] — corrupting http.request.method in the exported span.
